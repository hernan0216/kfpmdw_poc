...
class PwCompiler(Dag):
    """Converts a Dag object into a KFP executable.

    Compiler provides the tools to convert a Dag object into an
    executable script that uses the KFP DSL to create and upload a
    new pipeline.

    The Dag object is assumed to provide all the necessary information
    (environment, configuration, etc...) for the script to be compiled.
    """

    def __init__(self, pipeline: Dag):
...
#            for step in self.pipeline.components
#        ]
